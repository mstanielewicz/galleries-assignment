...
const getExhibitions = async ({
  ...
  if (search) {
    ...
    const dataResponse = await fetch(
      `https://api.artic.edu/api/v1/exhibitions?ids=${ids.join(
        ","
      )}&fields=id,description,gallery_title,is_featured,title,type&limit=30&page=${page}`
    );
    ...
    return {
      ...data,
      pagination: { total_pages: idsData.pagination.total_pages },
    };
  }
...
const useExhibitions = ({ page, search }: UseExhibitionsParams) => {
  ...
  return {
    ...rest,
    data: {
      rows: data?.data || ([] as ExhibitionData[]),
